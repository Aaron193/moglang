...
extract_dir="$(mktemp -d "${TMPDIR:-/tmp}/kelvra-vsix.XXXXXX")"
trap 'rm -rf "$extract_dir"' EXIT

(cd "$extract_dir" && cmake -E tar xvf "$vsix" >/dev/null)
extension_dir="$extract_dir/extension"
server_name=kelvra-lsp
[[ "$target" == win32-* ]] && server_name=kelvra-lsp.exe
...
KELVRA_REQUIRE_STATIC_OPENSSL=1 bash "$repo_root/scripts/inspect_native_dependencies.sh" "$server"
