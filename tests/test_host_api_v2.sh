...
KELVRA="$BUILD_DIR/interpreter"

if [[ ! -x "$KELVRA" ]]; then
    echo "Interpreter not found at $KELVRA" >&2
    exit 1
fi

OUTPUT="$($KELVRA "$SCRIPT_DIR/sample_host_api_v2.kel" 2>&1)"
EXPECTED=$'11\n13\n22\nfalse\n12\n11\n[0, 127, 255]\ntrue\ntrue\ntrue\ntrue\ntrue\ntrue'
...
LEGACY_OUTPUT="$($KELVRA "$SCRIPT_DIR/sample_import_native_package.kel" 2>&1)"
