...
std::string decodeUriPath(std::string_view uri) {
    ...
    // `file://C:/...` form that earlier Kelvra tooling emitted.
...
class KelvraLspServer {
   public:
    explicit KelvraLspServer(std::vector<std::string> packageSearchPaths)
   ...
   private:
    ...
    void handleMessage(const JsonObject& message) {
        ...
        if (*method == "kelvra/installProjectDependencies" && id != nullptr) {
        ...
        if (params != nullptr) {
            if (const JsonValue* optionsValue =
                ...
                if (const auto options = asObject(*optionsValue);
                    options.has_value()) {
                    if (const JsonValue* kelvraValue =
                            getObjectValue(options->get(), "kelvra")) {
                        if (const auto kelvra = asObject(*kelvraValue);
                            kelvra.has_value()) {
    ...
    bool ensureInstallForProjectRoot(const std::string& root,
        ...
        const std::filesystem::path manifestPath =
            std::filesystem::path(root) / "kelvra.toml";
        std::error_code ec;
        ...
        const std::string message = "Could not install Kelvra dependencies for '" +
                                    root + "': " + error;
    ...
    void handleInstallProjectDependencies(const JsonValue& id,
        ...
        if (root.empty()) {
            sendErrorResponse(id, -32602,
                              "No Kelvra project root was supplied or discovered.");
            return;
        }
    ...
    void handleWatchedFilesChanged(const JsonObject& params) {
        ...
        for (const auto& value : changes->get()) {
            ...
            const std::string path = canonicalizePath(decodeUriPath(*uri));
            if (std::filesystem::path(path).filename() == "kelvra.toml") {
                std::string root;
            ...
            if (std::filesystem::path(path).extension() != ".kel") {
                continue;
            }
    ...
    void rebuildWorkspaceIndex() {
        ...
        for (const auto& root : m_workspaceRoots) {
            ...
            if (std::filesystem::is_regular_file(root, ec)) {
                if (std::filesystem::path(root).extension() == ".kel") {
                    addFile(root);
                }
                continue;
            }
    ...
    std::vector<std::string> collectWorkspaceFiles() const {
        ...
        for (const auto& [uri, document] : m_documents) {
            (void)uri;
            if (std::filesystem::path(document.path).extension() != ".kel") {
                continue;
            }
    ...
    void sendInitializeResponse(const JsonValue* id) {
        ...
        result["serverInfo"] = JsonValue(JsonObject{
            {"name", JsonValue(std::string("kelvra-lsp"))},
            {"version", JsonValue(std::string(KELVRA_RUNTIME_VERSION))},
        });
        result["kelvra"] = JsonValue(JsonObject{
            {"serverVersion", JsonValue(std::string(KELVRA_RUNTIME_VERSION))},
            {"toolingProtocolVersion",
             JsonValue(std::string(KELVRA_TOOLING_PROTOCOL_VERSION))},
    ...
    void sendPublishDiagnostics(const std::string& uri,
        ...
        for (const auto& diagnostic : diagnostics) {
            ...
            item["source"] = JsonValue(std::string("kelvra"));
...
#endif
    KelvraLspServer server(defaultPackageSearchPaths(argv[0]));
    return server.run();
}
