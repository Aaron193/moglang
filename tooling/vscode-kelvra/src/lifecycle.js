...
class ServerInstance {
  ...
  relevantDocument() {
    ...
    if (active?.languageId === "kelvra" && this.contains(active.uri)) return active;
    return vscode.workspace.textDocuments.find(
      (document) => document.languageId === "kelvra" && this.contains(document.uri)
    );
  }
  ...
  discovery(allowFallback = false) {
    const config = vscode.workspace.getConfiguration("kelvra", this.workspaceFolder?.uri);
    ...
    const result = discoverServer({
      ...
      developmentMode: process.env.KELVRA_LSP_DEVELOPMENT === "1",
      developmentServerPath: process.env.KELVRA_SERVER_PATH,
  ...
  async start(options = {}) {
    ...
    try {
      ...
      if (!this.compatibility.compatible) {
        ...
        await this.fail(`Incompatible Kelvra language server: ${this.compatibility.reason}.`);
        return;
      }
  ...
  async fail(message, invalidExplicit) {
    ...
    void (async () => {
      const choice = await vscode.window.showErrorMessage(`Kelvra: ${message}`, ...actions);
...
class ServerManager {
  constructor(context) {
    ...
    this.output = vscode.window.createOutputChannel("Kelvra Language Support");
    this.status = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 10);
    this.status.command = "kelvra.showServerStatus";
    this.status.show();
    context.subscriptions.push(this.output, this.status);
    this.log(`Kelvra extension ${context.extension.packageJSON.version} activated.`);
    if (process.env.KELVRA_SERVER_PATH && process.env.KELVRA_LSP_DEVELOPMENT !== "1") {
      this.log("Ignored KELVRA_SERVER_PATH because KELVRA_LSP_DEVELOPMENT is not 1.");
    }
  }
  ...
  activeInstance() {
    ...
    if (document?.languageId === "kelvra") return this.instanceForUri(document.uri);
    return this.instances.values().next().value;
  }

  async start() {
    const documents = vscode.workspace.textDocuments.filter((document) => document.languageId === "kelvra");
  ...
  async openDocument(document) {
    if (document.languageId === "kelvra") await this.instanceForUri(document.uri).start();
  }
  ...
  updateStatus() {
    ...
    const display = {
      starting: ["$(sync~spin) Kelvra", "Kelvra language server is starting"],
      ready: ["$(check) Kelvra", "Kelvra language server is ready"],
      failed: ["$(error) Kelvra", "Kelvra language server failed; click for details"],
      stopped: ["$(circle-slash) Kelvra", "Kelvra language server is stopped"]
    }[state];
  ...
  async showStatus() {
    ...
    const message = rows.join("\n\n") || "No Kelvra language server has been created.";
  ...
  async selectServer(instance = this.activeInstance()) {
    const selection = await vscode.window.showOpenDialog({
      title: "Select the kelvra-lsp executable",
      canSelectFiles: true,
    ...
    if (!validation.valid) {
      void vscode.window.showErrorMessage(
        `Kelvra: ${selection[0].fsPath} is not a valid language server executable (${validation.reason}).`
      );
      return;
    }
    const target = instance?.workspaceFolder?.uri;
    const config = vscode.workspace.getConfiguration("kelvra", target);
  ...
  async installDependencies() {
    ...
    if (!instance?.client || instance.state !== "ready") {
      void vscode.window.showErrorMessage("Kelvra: start the language server before installing dependencies.");
      return;
    }
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: "Installing Kelvra project dependencies", cancellable: true },
      async (_progress, token) => {
        try {
          const request = instance.client.sendRequest(
            "kelvra/installProjectDependencies",
            { projectRoot: instance.selection.cwd },
            token
          );
          const result = await request;
          void vscode.window.showInformationMessage(result?.message || "Kelvra dependencies installed.");
        } catch (error) {
          this.log(`Dependency installation failed: ${error.message || error}`);
          const action = await vscode.window.showErrorMessage(
            `Kelvra dependency installation failed: ${error.message || error}`,
            "Open Log"
          );
          if (action === "Open Log") this.output.show(true);
        }
