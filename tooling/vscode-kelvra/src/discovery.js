...
const PROJECT_MARKERS = ["kelvra.toml", "package.toml"];

function executableName(platform = process.platform) {
  return platform === "win32" ? "kelvra-lsp.exe" : "kelvra-lsp";
}
...
function discoverServer(options = {}) {
  ...
  add(developmentPath, "KELVRA_SERVER_PATH development override", true);

  const configured = resolveConfiguredPath(options.configuredPath, activeWorkspacePath);
  add(configured, "kelvra.serverPath", true);
  ...
  if (options.runtimePath) {
    ...
    add(path.join(runtimeDirectory, executable), "installed Kelvra runtime");
    add(path.join(runtimeDirectory, "bin", executable), "installed Kelvra runtime");
  }

  // A `kelvra` executable on PATH identifies an installed runtime. Its sibling LSP
  // is checked here, before workspace build products as required by the plan.
  const runtimeExecutable = platform === "win32" ? "kelvra.exe" : "kelvra";
  for (const runtime of pathCandidates(options.pathValue, runtimeExecutable)) {
    if (validateExecutable(runtime, options).valid) {
      add(path.join(path.dirname(runtime), executable), "installed Kelvra runtime");
    }
  }
