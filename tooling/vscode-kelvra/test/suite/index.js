...
async function run() {
  const uri = vscode.Uri.file(path.resolve(__dirname, "..", "fixtures", "workspace", "main.kel"));
  const document = await vscode.workspace.openTextDocument(uri);
  await vscode.window.showTextDocument(document);
  assert.equal(document.languageId, "kelvra", ".kel files must use the Kelvra language mode");

  const extension = vscode.extensions.getExtension("kelvralang.vscode-kelvra");
  assert.ok(extension, "extension is registered");
  const api = await extension.activate();
  assert.ok(extension.isActive, "extension activates for a Kelvra document");
  const commands = await vscode.commands.getCommands(true);
  for (const command of [
    "kelvra.showServerStatus",
    "kelvra.restartServer",
    "kelvra.selectServer",
    "kelvra.openServerLog",
  ...
  try {
    ...
    if (process.env.KELVRA_SERVER_PATH) {
      assert.equal(instance.selection.path, path.resolve(process.env.KELVRA_SERVER_PATH));
    } else {
    ...
    assert.ok(
      ...
      "completion returns the expected Kelvra builtin"
    );
