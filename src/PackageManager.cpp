...
constexpr const char* kGeneratorVersion = MOG_RUNTIME_VERSION;
constexpr const char* kCurrentRuntimeVersion = MOG_RUNTIME_VERSION;
...
bool currentRuntimeSatisfiesRequirement(std::string_view requirementText,
    ...
    if (!parseVersionRequirement(requirementText, requirement, outError)) {
        if (!packageId.empty()) {
            outError = "Package '" + std::string(packageId) +
                       "' has invalid mog_runtime requirement: " + outError;
        }
        return false;
    }
    ...
    if (!packageId.empty()) {
        outError = "Package '" + std::string(packageId) + "' requires mog_runtime '" +
                   std::string(requirementText) + "', but this toolchain provides '" +
                   kCurrentRuntimeVersion + "'.";
    } else {
        outError = "Current toolchain version does not satisfy mog_runtime '" +
...
std::string canonicalRegistryIndexPayload(std::vector<RegistryIndexRecord> records,
    ...
    for (const auto& record : records) {
        ...
        if (!record.mogRuntime.empty()) {
            out << "mog_runtime = " << quoteTomlString(record.mogRuntime) << "\n";
        }
...
bool downloadUrlToFileDetailed(std::string_view url, const std::string& token,
                               ...
                               std::string& outError) {
    std::filesystem::path tempDir;
    if (!createTemporaryDirectory("mog-http-download", tempDir, outError)) {
        return false;
    }
    ScopedPathCleanup cleanup{tempDir};
...
bool uploadFileToUrlDetailed(const std::filesystem::path& source,
                             ...
                             std::string& outError) {
    std::filesystem::path tempDir;
    if (!createTemporaryDirectory("mog-http-upload", tempDir, outError)) {
        return false;
    }
    ScopedPathCleanup cleanup{tempDir};
    const std::filesystem::path headerPath = tempDir / "headers.txt";
...
bool writeInstallRegistryFile(const std::filesystem::path& outputPath,
    ...
    for (const auto& entry : entries) {
        ...
        if (!entry.mogRuntime.empty()) {
            out << "mog_runtime = " << quoteTomlString(entry.mogRuntime) << "\n";
        }
...
bool discoverHostedRegistryService(const EffectiveRegistryProfile& profile,
    ...
    std::filesystem::path tempDir;
    if (!createTemporaryDirectory("mog-registry-service", tempDir, outError)) {
        return false;
    }
    ScopedPathCleanup cleanup{tempDir};
...
bool resolveRegistryPackageNode(
    ...
    if (node.entry.mogRuntime.empty()) {
        node.entry.mogRuntime = record.mogRuntime;
    }
    if (!currentRuntimeSatisfiesRequirement(node.entry.mogRuntime, packageId,
                                            outError)) {
        return false;
    }
...
bool validatePackageEntry(const PackageRegistryEntry& entry,
                          const std::string& projectRoot,
                          std::string& outError) {
    if (!currentRuntimeSatisfiesRequirement(entry.mogRuntime, entry.packageId,
                                            outError)) {
        return false;
    }
...
bool packageEntryMatchesResolution(const PackageRegistryEntry& expected,
    ...
    return expected.packageId == actual.packageId &&
           ...
           expected.mogRuntime == actual.mogRuntime &&
...
bool validateLockedEntriesAgainstManifest(
    ...
    for (const auto& entry : lockedEntries) {
        ...
        if (std::find(groups.begin(), groups.end(), "dev") != groups.end()) {
            ...
            if (!options.includeDevDependencies) {
                outError =
                    "mog.lock is out of date for the requested dependency groups. Run 'mog update' to refresh it.";
                return false;
            }
        }
...
bool publishProjectPackage(const std::string& projectRoot,
    ...
    if (existingRecordIt == records.end()) {
        ...
        record.mogRuntime = packageEntry.mogRuntime;
        if (packageEntry.kind == "native") {
    ...
    if (!uploadFileToUrlDetailed(indexPath, registryLocation.remoteIndexUrl, token,
        ...
        if (indexWriteResponse.statusCode == 412) {
            outError = "Hosted publish failed because registry index.toml changed on the server. Rerun 'mog publish'.";
