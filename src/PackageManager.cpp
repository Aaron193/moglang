...
#include "NativePackageAPIEmbedded.hpp"
...
constexpr const char* kLockfileName = "kelvra.lock";
constexpr const char* kInstallRegistryFileName = "registry.toml";
constexpr const char* kPackageApiFileName = "package.api.kel";
constexpr const char* kRegistryServiceFileName = "registry-service.v1";
constexpr const char* kRegistryTrustFileName = "registry-trust.v1";
constexpr const char* kLockfileHeader = "# Generated by kelvra install. Do not edit by hand.";
constexpr const char* kCacheMetadataFileName = ".metadata.toml";
constexpr const char* kGeneratorVersion = KELVRA_RUNTIME_VERSION;
constexpr const char* kCurrentRuntimeVersion = KELVRA_RUNTIME_VERSION;
...
std::string defaultProjectName(const std::string& projectRoot) {
    ...
    return stem.empty() ? "kelvra-project" : stem;
}
...
struct LoadedRegistryTrustMetadata {
    std::filesystem::path trustPath;
    bool isRemote = false;
    std::string schemaVersion = "registry-trust.v1";
    std::string signingKeyId;
    std::string signatureBase64;
    std::vector<RegistryTrustRecord> records;
};
...
struct HostedRegistryServiceConfig {
    bool enabled = false;
...
std::string hostedRegistryBootstrapHint(std::string_view alias, bool isRemote) {
    ...
    return " Run 'kelvra registry trust " + std::string(alias) +
    ...
    if (toolchainPath.empty() && manifest != nullptr &&
        ...
        if (toolchainPath.empty()) {
            const std::string envName =
                "KELVRA_CMAKE_TOOLCHAIN_" + normalizeEnvKey(outSelection.target);
        ...
        if (toolchainPath.empty()) {
            const std::filesystem::path projectKelvraToolchain =
                std::filesystem::path(projectRoot) / ".kelvra" / "toolchains" /
                (outSelection.target + ".cmake");
            if (fileExists(projectKelvraToolchain)) {
                toolchainPath = projectKelvraToolchain;
                outSelection.sourceDescription =
                    normalizeRelativePath(projectKelvraToolchain.lexically_relative(
...
bool currentRuntimeSatisfiesRequirement(std::string_view requirementText,
                                        std::string_view packageId,
                                        std::string& outError) {
    ...
    if (!parseVersionRequirement(requirementText, requirement, outError)) {
        if (!packageId.empty()) {
            outError = "Package '" + std::string(packageId) +
                       "' has invalid kelvra_runtime requirement: " + outError;
        }
        return false;
    }

    SemanticVersion currentRuntime;
    if (!parseSemanticVersion(kCurrentRuntimeVersion, currentRuntime)) {
        outError = "Internal error: invalid current runtime version.";
        return false;
    }
    if (versionSatisfiesRequirement(currentRuntime, requirement)) {
        return true;
    }

    if (!packageId.empty()) {
        outError = "Package '" + std::string(packageId) + "' requires kelvra_runtime '" +
                   std::string(requirementText) + "', but this toolchain provides '" +
                   kCurrentRuntimeVersion + "'.";
    } else {
        outError = "Current toolchain version does not satisfy kelvra_runtime '" +
...
std::filesystem::path remoteRegistryTrustCachePath(std::string_view alias,
                                                   std::string_view rootUrl) {
    return registryCacheRoot() / "trust" / normalizeEnvKey(alias) /
           hashString(rootUrl) / kRegistryTrustFileName;
}
...
std::string canonicalRegistryIndexPayload(std::vector<RegistryIndexRecord> records,
    ...
    for (const auto& record : records) {
        ...
        if (!record.license.empty()) {
            out << "license = " << quoteTomlString(record.license) << "\n";
        }
        if (!record.kelvraRuntime.empty()) {
            out << "kelvra_runtime = " << quoteTomlString(record.kelvraRuntime) << "\n";
        }
...
std::string canonicalRegistryTrustPayload(std::vector<RegistryTrustRecord> records) {
    std::sort(records.begin(), records.end(),
              [](const RegistryTrustRecord& lhs, const RegistryTrustRecord& rhs) {
                  return lhs.keyId < rhs.keyId;
              });

    std::ostringstream out;
    out << "schema_version = " << quoteTomlString("registry-trust.v1") << "\n";
    for (const auto& record : records) {
        out << "\n[[key]]\n";
        out << "key_id = " << quoteTomlString(record.keyId) << "\n";
        out << "public_key = " << quoteTomlString(record.publicKeyDerBase64)
            << "\n";
        out << "status = " << quoteTomlString(record.status) << "\n";
        if (!record.reason.empty()) {
            out << "reason = " << quoteTomlMultilineString(record.reason) << "\n";
        }
    }
    return out.str();
}
...
bool findTrustedRegistryKey(const EffectiveRegistryProfile& profile,
                            std::string_view signingKeyId,
                            RegistryTrustedKey& outKey,
                            std::string& outError) {
    outKey = RegistryTrustedKey{};
    if (std::find(profile.revokedKeyIds.begin(), profile.revokedKeyIds.end(),
                  std::string(signingKeyId)) != profile.revokedKeyIds.end()) {
        outError = "Registry '" + profile.config.alias +
                   "' was signed by revoked key '" +
                   std::string(signingKeyId) + "'.";
        return false;
    }
    for (const std::string& rawTrustedKey : profile.config.trustedKeys) {
    ...
    outError = "Registry '" + profile.config.alias +
               "' was signed by unknown key '" +
               std::string(signingKeyId) + "'.";
    return false;
}

bool verifyRegistrySignatureEnvelope(const EffectiveRegistryProfile& profile,
                                     std::string_view signingKeyId,
                                     std::string_view signatureBase64,
                                     std::string_view canonicalPayload,
                                     std::string& outError) {
    if (!registryRequiresTrustedSignatures(profile.config, profile.isRemote)) {
        return true;
    }
    if (signingKeyId.empty() || signatureBase64.empty()) {
        outError = "Registry '" + profile.config.alias +
                   "' requires a signed index, but the registry metadata is unsigned." +
                   hostedRegistryBootstrapHint(profile.config.alias,
                                              profile.isRemote);
        return false;
    }
    if (profile.config.trustedKeys.empty()) {
        outError = "Registry '" + profile.config.alias +
    ...
    RegistryTrustedKey trustedKey;
    if (!findTrustedRegistryKey(profile, signingKeyId, trustedKey, outError)) {
        outError += hostedRegistryBootstrapHint(profile.config.alias,
                                                profile.isRemote);
        return false;
    }
    ...
    if (!verifyEd25519Message(trustedKey.publicKeyDerBase64, canonicalPayload,
        ...
        outError = "Registry '" + profile.config.alias +
                   "' signature verification failed: " + outError;
        return false;
    }
    return true;
}
...
bool verifyRegistryArtifactSignature(const ProjectPolicyConfig& policy,
    ...
    const bool requireSignature =
        registryArtifactSignaturesRequired(policy, profile.config);
    if (!requireSignature) {
    ...
    if (entry.registryKeyId.empty() || entry.artifactSignature.empty()) {
        outError = "Registry artifact for '" + entry.packageId + "@" +
                   ...
                   "' must be signed.";
        if (requireSignature && profile.config.trustedKeys.empty()) {
            outError += " Configure trusted_keys for registry '" +
    ...
    RegistryTrustedKey trustedKey;
    if (!findTrustedRegistryKey(profile, entry.registryKeyId, trustedKey,
                                outError)) {
        if (profile.isRemote) {
            outError += hostedRegistryBootstrapHint(profile.config.alias,
                                                    profile.isRemote);
...
std::filesystem::path installRoot(const std::filesystem::path& projectRoot) {
    return projectRoot / ".kelvra" / "install";
}
...
std::filesystem::path userCacheRoot() {
    if (const char* cacheDir = std::getenv("KELVRA_CACHE_DIR");
    ...
    if (const char* xdgCache = std::getenv("XDG_CACHE_HOME");
        ...
        return std::filesystem::path(xdgCache) / "kelvra" / "packages";
    }

    if (const char* home = std::getenv("HOME"); home != nullptr && *home != '\0') {
        return std::filesystem::path(home) / ".cache" / "kelvra" / "packages";
    }

    return std::filesystem::temp_directory_path() / "kelvra-cache" / "packages";
}
...
std::filesystem::path projectFallbackCacheRoot(
    ...
    return projectRoot / ".kelvra" / "cache" / "packages" / "local";
}
...
std::filesystem::path userConfigRoot() {
    if (const char* xdgConfig = std::getenv("XDG_CONFIG_HOME");
        ...
        return std::filesystem::path(xdgConfig) / "kelvra";
    }
    if (const char* home = std::getenv("HOME"); home != nullptr && *home != '\0') {
        return std::filesystem::path(home) / ".config" / "kelvra";
    }
    return std::filesystem::temp_directory_path() / "kelvra-config";
}
...
std::string contentAddressedRegistryArtifactPath(std::string_view digest) {
    const std::string prefix = "sha256:";
    if (digest.rfind(prefix, 0) != 0 || digest.size() <= prefix.size()) {
        return "blobs/unknown";
    }
    return "blobs/sha256/" + std::string(digest.substr(prefix.size()));
}
...
bool runCapturedSystemCommand(const std::string& command, std::string& outText,
    ...
    if (!createTemporaryDirectory("kelvra-cmd", tempDir, outError)) {
        return false;
    }
...
bool parseHttpStatusCode(std::string_view rawStatus, int& outStatus,
                         std::string& outError) {
    const std::string trimmed = trim(rawStatus);
    if (trimmed.size() != 3 ||
        ...
        outError = "curl did not return a valid HTTP status code.";
        return false;
    }
    outStatus = std::stoi(trimmed);
    return true;
}

bool loadHttpResponseMetadata(const std::filesystem::path& headerPath,
                              std::string_view rawStatus,
                              HttpResponseMetadata& outMetadata,
                              std::string& outError) {
    outMetadata = HttpResponseMetadata{};
    if (!parseHttpStatusCode(rawStatus, outMetadata.statusCode, outError)) {
        return false;
    }

    std::ifstream file(headerPath);
    if (!file) {
        outError = "Could not read captured HTTP response headers from '" +
                   headerPath.string() + "'.";
        return false;
    }
    ...
    while (std::getline(file, line)) {
        ...
        if (line.empty()) {
            if (!currentEtag.empty()) {
                outMetadata.etag = currentEtag;
            }
            continue;
        }
        ...
        if (key == "etag") {
            currentEtag = trim(line.substr(colon + 1));
        }
    }
    if (!currentEtag.empty()) {
        outMetadata.etag = currentEtag;
...
std::string formatHttpFailureMessage(const HttpResponseMetadata& response,
                                     const std::filesystem::path& bodyPath) {
    std::string message =
        "HTTP " + std::to_string(response.statusCode);
    const std::string description = httpStatusDescription(response.statusCode);
    if (!description.empty()) {
        message += " " + description;
    }
    const std::string body = trim(readFileText(bodyPath));
    if (!body.empty()) {
        message += ": " + body;
    }
    return message;
...
bool downloadUrlToFileDetailed(std::string_view url, const std::string& token,
                               const std::filesystem::path& destination,
                               const std::vector<std::string>& extraHeaders,
                               HttpResponseMetadata* outResponse,
                               std::string& outError) {
    std::filesystem::path tempDir;
    if (!createTemporaryDirectory("kelvra-http-download", tempDir, outError)) {
        return false;
    }
    ScopedPathCleanup cleanup{tempDir};
    const std::filesystem::path headerPath = tempDir / "headers.txt";
    const std::filesystem::path bodyPath = tempDir / "body.bin";

    std::ostringstream command;
    command << "curl -sSL -D " << shellQuote(headerPath.string()) << " ";
    if (!token.empty()) {
        command << "-H "
    ...
    command << "-o " << shellQuote(bodyPath.string()) << " "
    ...
    HttpResponseMetadata response;
    if (!loadHttpResponseMetadata(headerPath, output, response, outError)) {
        return false;
    }
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
        outError = formatHttpFailureMessage(response, bodyPath);
        return false;
    }

    if (!copyFileReplacing(bodyPath, destination, outError)) {
        return false;
    }
    return true;
}
...
bool downloadUrlToFile(std::string_view url, const std::string& token,
                       const std::filesystem::path& destination,
                       std::string& outError) {
    return downloadUrlToFileDetailed(url, token, destination, {}, nullptr,
                                     outError);
}

bool uploadFileToUrlDetailed(const std::filesystem::path& source,
                             std::string_view url,
                             ...
                             std::string& outError) {
    std::filesystem::path tempDir;
    if (!createTemporaryDirectory("kelvra-http-upload", tempDir, outError)) {
        return false;
    }
    ScopedPathCleanup cleanup{tempDir};
    const std::filesystem::path headerPath = tempDir / "headers.txt";
    const std::filesystem::path bodyPath = tempDir / "body.txt";

    std::ostringstream command;
    command << "curl -sSL -X PUT -D " << shellQuote(headerPath.string()) << " ";
    if (!token.empty()) {
        command << "-H "
                << shellQuote("Authorization: Bearer " + token) << " ";
    }
...
bool uploadFileToUrl(const std::filesystem::path& source, std::string_view url,
    ...
    return uploadFileToUrlDetailed(source, url, token, {}, nullptr, outError);
}

bool uploadDirectoryToUrlDetailed(const std::filesystem::path& sourceDir,
...
bool uploadDirectoryToUrl(const std::filesystem::path& sourceDir,
                          std::string_view destinationUrl,
                          const std::string& token, std::string& outError) {
    return uploadDirectoryToUrlDetailed(sourceDir, destinationUrl, token, {},
                                        outError);
}
...
bool loadPackageEntryFromDir(const std::filesystem::path& packageDir,
    ...
    if (outEntry.kind == "source") {
        std::filesystem::path entryPath = manifest.sourceEntry.empty()
                                              ? packageDir / "src" / "main.kel"
                                              : packageDir / manifest.sourceEntry;
...
bool writeInstallRegistryFile(const std::filesystem::path& outputPath,
    ...
    out << kLockfileHeader << "\n";
    out << "schema_version = " << quoteTomlString("install.v4") << "\n";
    out << "generator_version = " << quoteTomlString(kGeneratorVersion) << "\n";
    for (const auto& entry : entries) {
        ...
        if (!entry.license.empty()) {
            out << "license = " << quoteTomlString(entry.license) << "\n";
        }
        if (!entry.kelvraRuntime.empty()) {
            out << "kelvra_runtime = " << quoteTomlString(entry.kelvraRuntime) << "\n";
        }
...
bool writeLockfile(const std::filesystem::path& outputPath,
    ...
    out << kLockfileHeader << "\n";
    out << "schema_version = " << quoteTomlString("lock.v4") << "\n";
    out << "generator_version = " << quoteTomlString(kGeneratorVersion) << "\n";
    for (const auto& entry : entries) {
        out << "\n[[package]]\n";
        out << "name = " << quoteTomlString(entry.importName) << "\n";
...
bool loadCacheMetadataFile(const std::filesystem::path& metadataPath,
    ...
    while (std::getline(file, line)) {
        ...
        if (key == "module") {
            ...
            outMetadata.license = parsed;
        } else if (key == "kelvra_runtime") {
            outMetadata.kelvraRuntime = parsed;
...
bool cacheMetadataMatchesEntry(const CacheEntryMetadata& metadata,
    ...
    return metadata.module == entry.module &&
           ...
           metadata.kelvraRuntime == entry.kelvraRuntime &&
...
bool loadRegistryTrustMetadata(const std::string& projectRoot,
                               const EffectiveRegistryProfile& profile,
                               bool offline,
                               bool& outFound,
                               LoadedRegistryTrustMetadata& outMetadata,
                               std::string& outError);

bool applyRegistryTrustMetadata(const LoadedRegistryTrustMetadata& metadata,
...
bool resolveEffectiveRegistryProfile(const std::string& projectRoot,
    ...
    if (includeTrustMetadata && !outProfile.config.trustedKeys.empty()) {
        bool found = false;
        LoadedRegistryTrustMetadata metadata;
        if (!loadRegistryTrustMetadata(projectRoot, outProfile, offline, found,
                                       metadata, outError)) {
            return false;
        }
        if (found && !applyRegistryTrustMetadata(metadata, outProfile, outError)) {
            return false;
        }
    }
...
bool parseHostedRegistryServiceFile(const std::filesystem::path& servicePath,
                                    HostedRegistryServiceConfig& outService,
                                    std::string& outError) {
    outService = HostedRegistryServiceConfig{};
    std::ifstream file(servicePath);
    if (!file) {
        outError = "Could not open hosted registry service file '" +
                   servicePath.string() + "'.";
        return false;
    }
    ...
    while (std::getline(file, line)) {
        ...
        const size_t equals = content.find('=');
        if (equals == std::string::npos) {
            outError = "Invalid hosted registry service line " +
                       std::to_string(lineNumber) + ": expected key = value.";
            return false;
        }
        const std::string key = trim(std::string_view(content).substr(0, equals));
        ...
        if (key == "schema_version") {
            schemaVersion = parsed;
        } else if (key == "artifact_upload_mode") {
    ...
    if (schemaVersion.empty()) {
        outError =
            "Hosted registry service file must declare schema_version = \"registry-service.v1\".";
        return false;
    }
    if (schemaVersion != "registry-service.v1") {
        outError = "Hosted registry service uses unsupported schema '" +
                   schemaVersion + "'.";
        return false;
    }
    if (artifactUploadMode != "content-addressed") {
        outError =
            "Hosted registry service must declare artifact_upload_mode = \"content-addressed\".";
        return false;
    }
    if (indexUpdateMode != "etag-if-match") {
        outError =
            "Hosted registry service must declare index_update_mode = \"etag-if-match\".";
        return false;
    }
...
bool discoverHostedRegistryPublishPolicy(
    ...
    std::filesystem::path tempDir;
    if (!createTemporaryDirectory("kelvra-registry-publish-policy", tempDir,
                                  outError)) {
        return false;
    }
    ScopedPathCleanup cleanup{tempDir};
...
bool discoverHostedRegistryService(const EffectiveRegistryProfile& profile,
                                   const RegistryLocation& location,
                                   HostedRegistryServiceConfig& outService,
                                   std::string& outError) {
    outService = HostedRegistryServiceConfig{};
    ...
    std::filesystem::path tempDir;
    if (!createTemporaryDirectory("kelvra-registry-service", tempDir, outError)) {
        return false;
    }
    ScopedPathCleanup cleanup{tempDir};
    const std::filesystem::path servicePath =
        tempDir / kRegistryServiceFileName;
    HttpResponseMetadata response;
    if (!downloadUrlToFileDetailed(joinUrlPath(location.remoteRootUrl,
                                               kRegistryServiceFileName),
                                   profile.token, servicePath, {}, &response,
                                   outError)) {
    ...
    if (!parseHostedRegistryServiceFile(servicePath, outService, outError)) {
        outError = "Registry '" + profile.config.alias +
                   "' service discovery failed: " + outError;
        return false;
    }
...
bool resolveRegistryAuthTokenForIndex(std::string_view alias,
    ...
    const std::string envAlias =
        "KELVRA_REGISTRY_TOKEN_" + normalizeEnvKey(alias);
    if (const char* specific = std::getenv(envAlias.c_str());
        specific != nullptr && *specific != '\0') {
        outToken = specific;
        return true;
    }
    if (const char* generic = std::getenv("KELVRA_REGISTRY_TOKEN");
...
bool loadRegistryTrustMetadata(const std::string& projectRoot,
                               ...
                               std::string& outError) {
    outFound = false;
    outMetadata = LoadedRegistryTrustMetadata{};
    outError.clear();

    RegistryLocation location;
    if (!resolveRegistryLocation(projectRoot, profile.config, location, outError)) {
        return false;
    }

    const std::filesystem::path trustPath =
        location.isRemote
            ? remoteRegistryTrustCachePath(profile.config.alias,
                                           location.remoteRootUrl)
            : location.localRootDir / kRegistryTrustFileName;
    if (location.isRemote) {
        if (!offline || !fileExists(trustPath)) {
            HttpResponseMetadata response;
            std::string downloadError;
            if (!downloadUrlToFileDetailed(
                    joinUrlPath(location.remoteRootUrl, kRegistryTrustFileName),
                    profile.token, trustPath, {}, &response, downloadError)) {
                if (response.statusCode == 404) {
                    std::string removeError;
                    if (!removePathIfExists(trustPath, removeError)) {
                        outError = "Could not clear stale cached registry trust metadata for '" +
                                   profile.config.alias + "': " + removeError;
                        return false;
                    }
                    return true;
                }
                outError = "Registry '" + profile.config.alias +
                           "' trust metadata download failed: " + downloadError;
                return false;
            }
        }
    } else if (!fileExists(trustPath)) {
        return true;
    }
    ...
    outMetadata.trustPath = trustPath;
    outMetadata.isRemote = location.isRemote;

    std::ifstream file(trustPath);
    if (!file) {
        outError = "Could not open registry trust metadata '" +
                   trustPath.string() + "'.";
        return false;
    }

    RegistryTrustRecord current;
    ...
    auto flushCurrent = [&]() -> bool {
        ...
        if (current.keyId.empty() || current.publicKeyDerBase64.empty() ||
            current.status.empty()) {
            outError = "Registry '" + profile.config.alias +
                       "' trust entries must define key_id, public_key, and status.";
            return false;
        }
        if (current.status != "active" && current.status != "revoked") {
            outError = "Registry '" + profile.config.alias +
                       "' trust entry for key '" + current.keyId +
                       "' must use status = \"active\" or \"revoked\".";
            return false;
        }
        outMetadata.records.push_back(current);
        current = RegistryTrustRecord{};
        hasCurrent = false;
        return true;
    };
    ...
    while (std::getline(file, line)) {
        ...
        const size_t equals = content.find('=');
        if (equals == std::string::npos) {
            outError = "Invalid registry trust line " + std::to_string(lineNumber) +
                       ": expected key = value.";
            return false;
        }
        ...
        if (!hasCurrent) {
            std::string parsed;
            if (!parseQuotedString(value, parsed, outError)) {
                outError = "Invalid registry trust line " +
                           std::to_string(lineNumber) + ": " + outError;
                return false;
            }
            if (key == "schema_version") {
                outMetadata.schemaVersion = parsed;
            } else if (key == "signing_key_id") {
                outMetadata.signingKeyId = parsed;
            } else if (key == "signature") {
                outMetadata.signatureBase64 = parsed;
            } else {
                outError = "Invalid registry trust line " +
                           std::to_string(lineNumber) +
                           ": entries must appear inside [[key]].";
                return false;
            }
            continue;
        }
        ...
        std::string parsed;
        if (key == "reason" && value.rfind("\"\"\"", 0) == 0) {
            if (!parseMultilineStringValue(value, file, parsed, lineNumber,
                                           outError)) {
                outError = "Invalid registry trust line " +
                           std::to_string(lineNumber) + ": " + outError;
                return false;
        ...
        if (key == "key_id") {
            current.keyId = parsed;
        } else if (key == "public_key") {
            current.publicKeyDerBase64 = parsed;
        } else if (key == "status") {
            current.status = parsed;
        } else if (key == "reason") {
            current.reason = parsed;
        } else {
            outError = "Unsupported registry trust field '" + key + "'.";
            return false;
        }
    }
    ...
    if (outMetadata.schemaVersion != "registry-trust.v1") {
        outError = "Registry '" + profile.config.alias +
                   "' uses unsupported trust schema '" +
                   outMetadata.schemaVersion + "'.";
        return false;
    }

    const std::string canonicalPayload =
        canonicalRegistryTrustPayload(outMetadata.records);
    if (!verifyRegistrySignatureEnvelope(profile, outMetadata.signingKeyId,
                                         outMetadata.signatureBase64,
...
bool applyRegistryTrustMetadata(const LoadedRegistryTrustMetadata& metadata,
                                EffectiveRegistryProfile& outProfile,
                                std::string& outError) {
    outProfile.revokedKeyIds.clear();
    std::unordered_set<std::string> seenKeyIds;
    std::vector<std::string> mergedTrustedKeys = outProfile.config.trustedKeys;

    for (const auto& record : metadata.records) {
        if (!seenKeyIds.insert(record.keyId).second) {
            outError = "Registry '" + outProfile.config.alias +
                       "' trust metadata contains duplicate key '" +
                       record.keyId + "'.";
            return false;
        }
        if (record.status == "active") {
            RegistryTrustedKey trustedKey;
            trustedKey.keyId = record.keyId;
            trustedKey.publicKeyDerBase64 = record.publicKeyDerBase64;
            mergedTrustedKeys.push_back(trustedRegistryKeySpec(trustedKey));
        } else if (record.status == "revoked") {
            outProfile.revokedKeyIds.push_back(record.keyId);
        }
    }

    outProfile.revokedKeyIds =
        sortedUniqueStrings(std::move(outProfile.revokedKeyIds));
    const std::unordered_set<std::string> revokedKeyIds(
        outProfile.revokedKeyIds.begin(), outProfile.revokedKeyIds.end());

    std::vector<std::string> filteredTrustedKeys;
    filteredTrustedKeys.reserve(mergedTrustedKeys.size());
    for (const std::string& rawTrustedKey : mergedTrustedKeys) {
        RegistryTrustedKey trustedKey;
        if (!parseTrustedRegistryKey(rawTrustedKey, trustedKey, outError)) {
            outError = "Registry '" + outProfile.config.alias +
                       "' has invalid trusted_keys: " + outError;
        ...
        if (revokedKeyIds.find(trustedKey.keyId) != revokedKeyIds.end()) {
            continue;
        }
        filteredTrustedKeys.push_back(rawTrustedKey);
    }

    outProfile.config.trustedKeys =
        sortedUniqueStrings(std::move(filteredTrustedKeys));
    if (!parseTrustedKeyIds(outProfile.config.trustedKeys,
                            outProfile.trustedKeyIds, outError)) {
        outError = "Registry '" + outProfile.config.alias +
                   "' has invalid trusted_keys: " + outError;
...
bool loadRegistryIndex(const std::string& projectRoot,
    ...
    if (!resolveEffectiveRegistryProfile(projectRoot, config,
                                         options != nullptr && options->offline,
                                         true, effectiveProfile, outError)) {
        return false;
    }
    ...
    if (location.isRemote) {
        ...
        if (!offline || !fileExists(configuredPath)) {
            HttpResponseMetadata response;
            if (!downloadUrlToFileDetailed(location.remoteIndexUrl,
                                           effectiveProfile.token,
                                           configuredPath, {}, &response,
                                           outError)) {
                outError = "Registry '" + config.alias +
                           "' index download failed: " + outError;
                return false;
            }
            outIndex.remoteIndexEtag = response.etag;
        }
    }
    ...
    if (outIndex.schemaVersion == "registry.v2" ||
        ...
        if (!verifyRegistrySignatureEnvelope(effectiveProfile,
            ...
            return false;
                                                 ...
                                                 location.isRemote)) {
        outError = "Registry '" + config.alias +
                   "' requires a signed registry index, but the registry is unsigned." +
...
bool loadRegistryAdvisories(const std::string& projectRoot,
    ...
    EffectiveRegistryProfile effectiveProfile;
    if (!resolveEffectiveRegistryProfile(projectRoot, config, options.offline,
                                         true, effectiveProfile, outError)) {
        return false;
    }
...
bool resolveRegistryPackageNode(
    ...
    if (node.entry.license.empty()) {
        node.entry.license = record.license;
    }
    if (node.entry.kelvraRuntime.empty()) {
        node.entry.kelvraRuntime = record.kelvraRuntime;
    }
    if (!currentRuntimeSatisfiesRequirement(node.entry.kelvraRuntime, packageId,
                                            outError)) {
        return false;
    }
...
bool stagePublishedNativeSourceArtifact(const std::filesystem::path& packagePath,
    ...
    if (!createTemporaryDirectory("kelvra-publish-native-source", outStagingDir,
                                  outError)) {
        return false;
    }
...
bool stagePublishedNativePrebuiltArtifact(const std::filesystem::path& packagePath,
    ...
    if (!createTemporaryDirectory("kelvra-publish-native", outStagingDir, outError)) {
        return false;
    }
...
bool stagePublishedNativePrebuiltArtifactFromDirectory(
    ...
    if (!fileExists(apiPath)) {
        outError = "Native publish artifact directory '" +
                   nativeArtifactDir.string() + "' is missing package.api.kel.";
        return false;
    }
...
bool writeGeneratedNativeBuildCMakeLists(const std::filesystem::path& outputPath,
    ...
    out << "cmake_minimum_required(VERSION 3.10)\n"
        << "project(kelvra_native_package LANGUAGES CXX)\n"
        << "set(CMAKE_CXX_STANDARD 17)\n"
        << "set(CMAKE_CXX_STANDARD_REQUIRED ON)\n"
        << "add_library(kelvra_native_package MODULE "
        << quoteTomlString((packageDir / "package.cpp").string()) << ")\n"
        << "target_include_directories(kelvra_native_package PRIVATE "
        << quoteTomlString(packageDir.string()) << ")\n"
        << "set_target_properties(kelvra_native_package PROPERTIES PREFIX \"\" OUTPUT_NAME \"package\")\n";
    return true;
}
...
bool buildNativePackageFromSource(const std::string& projectRoot,
    ...
    if (!createTemporaryDirectory("kelvra-native-build", buildDir, outError)) {
        return false;
    }
    ...
    if (!fileExists(packageCmake)) {
        if (!createTemporaryDirectory("kelvra-native-build-src", generatedSourceDir,
                                      outError)) {
            return false;
        }
...
bool validatePackageEntry(const PackageRegistryEntry& entry,
                          const std::string& projectRoot,
                          std::string& outError) {
    if (!currentRuntimeSatisfiesRequirement(entry.kelvraRuntime, entry.packageId,
                                            outError)) {
        return false;
    }
...
bool materializeCacheEntry(const PackageRegistryEntry& sourceEntry,
    ...
    if (sourceEntry.sourceType == "registry") {
        ...
        EffectiveRegistryProfile effectiveProfile;
        if (!resolveEffectiveRegistryProfile(projectRoot, *registry,
                                             options.offline, true,
                                             effectiveProfile, outError)) {
            return false;
        }
        if (!verifyRegistryArtifactSignature(manifest.policy, effectiveProfile,
...
bool collectResolvedPackages(
    ...
    if (options.includeBuildDependencies &&
        !appendDependencies(manifest.buildDependencies, "build")) {
        return false;
    }
...
bool packageEntryMatchesResolution(const PackageRegistryEntry& expected,
    ...
    return expected.packageId == actual.packageId &&
           ...
           expected.kelvraRuntime == actual.kelvraRuntime &&
...
bool dependencyMatchesLockedEntry(const DependencySpec& dependency,
    ...
    if (dependency.module.empty() && lockedEntry.importName != dependency.alias) {
        outError = "kelvra.lock is out of date for dependency '" + dependency.alias +
                   "'. Run 'kelvra update' to refresh it.";
        return false;
    }
...
bool validateLockedEntriesAgainstManifest(
        ...
        for (const auto& dependency : dependencies) {
            ...
            if (!dependency.git.empty()) {
                ...
                if (lockedIt == lockedEntries.end()) {
                    outError = "kelvra.lock is missing git dependency '" +
                               dependency.alias + "'.";
                    return false;
                }
            ...
            if (isPublishedDependency) {
                ...
                if (lockedIt == lockedById.end()) {
                    outError = "kelvra.lock is missing package '" +
                               dependency.packageId + "'.";
                    return false;
                }
                ...
                if (lockedIt->second.sourceType != "registry" ||
                    ...
                    outError = "kelvra.lock is out of date for dependency '" +
                               dependency.alias + "'. Run 'kelvra update' to refresh it.";
                    return false;
                }
                if (lockedIt->second.kind == "native") {
                    ...
                    if (lockedIt->second.selectedTarget != expectedTarget) {
                        outError = "kelvra.lock is pinned for native target '" +
                                   lockedIt->second.selectedTarget +
                                   "', but this install requires '" +
                                   expectedTarget +
                                   "'. Run 'kelvra update --target " + expectedTarget +
                                   "' to refresh it.";
                        return false;
                    }
                }
    ...
    if (options.includeBuildDependencies &&
        !validateDependencies(manifest.buildDependencies)) {
        return false;
    }

    bool hasDevGroup = false;
    bool hasBuildGroup = false;
    for (const auto& entry : lockedEntries) {
        const std::vector<std::string> groups =
            sortedUniqueStrings(entry.dependencyGroups);
        if (std::find(groups.begin(), groups.end(), "dev") != groups.end()) {
            hasDevGroup = true;
            if (!options.includeDevDependencies) {
                outError =
                    "kelvra.lock is out of date for the requested dependency groups. Run 'kelvra update' to refresh it.";
                return false;
            }
        }
        if (std::find(groups.begin(), groups.end(), "build") != groups.end()) {
            hasBuildGroup = true;
            if (!options.includeBuildDependencies) {
                outError =
                    "kelvra.lock is out of date for the requested dependency groups. Run 'kelvra update' to refresh it.";
    ...
    if (options.includeDevDependencies && !manifest.devDependencies.empty() &&
        !hasDevGroup) {
        outError =
            "kelvra.lock is out of date for the requested dependency groups. Run 'kelvra update' to refresh it.";
        return false;
    }
    if (options.includeBuildDependencies && !manifest.buildDependencies.empty() &&
        !hasBuildGroup) {
        outError =
            "kelvra.lock is out of date for the requested dependency groups. Run 'kelvra update' to refresh it.";
        return false;
    ...
    for (const auto& entry : lockedEntries) {
        ...
        if (!loadPackageEntryFromDir(std::filesystem::path(entry.sourcePath), current,
                                     nullptr, outError)) {
            outError = "kelvra.lock is out of date for package '" + entry.packageId +
                       "'. Run 'kelvra update' to refresh it.";
            return false;
        }
...
bool validateLockedGraph(const std::vector<PackageRegistryEntry>& resolvedEntries,
    ...
    if (resolvedEntries.size() != lockedEntries.size()) {
        outError = "kelvra.lock is out of date with kelvra.toml: resolved " +
    ...
    for (const auto& resolved : resolvedEntries) {
        ...
        if (lockedIt == lockedById.end()) {
            outError = "kelvra.lock is missing package '" + resolved.packageId + "'.";
            return false;
        }

        if (!packageEntryMatchesLocked(resolved, lockedIt->second)) {
            outError = "kelvra.lock is out of date for package '" +
                       resolved.packageId + "'. Run 'kelvra install' to refresh it.";
            return false;
        }
    }
...
bool loadProjectManifestData(const std::string& projectRoot,
    ...
    while (std::getline(file, line)) {
        ...
        if (section == Section::DEPENDENCIES) {
            ...
            outManifest.buildDependencies.push_back(std::move(dependency));
        }
    }
    ...
    if (outManifest.kind != "project") {
        outError = "Root kelvra.toml must declare kind = \"project\".";
        return false;
    }
...
bool writeProjectManifestData(const std::string& projectRoot,
    ...
    std::vector<DependencySpec> buildDependencies = manifest.buildDependencies;
    ...
    sortDependencies(buildDependencies);
    ...
    writeDependencyTable(out, "build-dependencies", buildDependencies);
    return true;
}
...
bool removeProjectDependency(const std::string& projectRoot,
    ...
    if (!removeDependency(manifest.dependencies) &&
        ...
        !removeDependency(manifest.buildDependencies)) {
        outError = "No dependency named '" + alias +
                   "' exists in [dependencies], [dev-dependencies], or [build-dependencies].";
        return false;
    }
...
bool publishProjectPackage(const std::string& projectRoot,
    ...
    EffectiveRegistryProfile effectiveProfile;
    if (!resolveEffectiveRegistryProfile(projectRoot, *registry, false, true,
                                         effectiveProfile, outError)) {
        return false;
    }
    ...
    if (registryLocation.isRemote) {
        if (effectiveProfile.token.empty()) {
            outError = "Hosted publish requires a stored token. Run 'kelvra login " +
    ...
    if (registryLocation.isRemote) {
        if (!createTemporaryDirectory("kelvra-publish-registry", stagedRegistryRoot,
                                      outError)) {
            return false;
        }
    ...
    if (hostedService.enabled && hostedService.conditionalIndexUpdates &&
        loadedExistingIndex && existingIndex.remoteIndexEtag.empty()) {
        outError = "Registry '" + effectiveRegistryAlias +
                   "' advertises registry-service.v1 but did not return an ETag for index.toml.";
        return false;
    }
    ...
    if (packageEntry.kind == "native") {
        ...
        if (!createTemporaryDirectory("kelvra-publish-source", sourceArtifactSource,
                                      outError)) {
            return false;
        }
        stagedSourceArtifactCleanup.path = sourceArtifactSource;
    ...
    const bool useHostedServicePaths =
        ...
        hostedService.contentAddressedArtifacts;
    const std::string relativeSourceArtifactPath =
        useHostedServicePaths
            ? contentAddressedRegistryArtifactPath(sourceArtifactDigest)
            : normalizeRelativePath(
                  (packageEntry.kind == "native"
                       ? (std::filesystem::path("packages") /
                          packageEntry.packageNamespace / packageEntry.packageName /
                          packageEntry.version / "source")
                       : (std::filesystem::path("packages") /
                          packageEntry.packageNamespace / packageEntry.packageName /
                          packageEntry.version))
    ...
    const std::filesystem::path sourceArtifactDir =
        registryRoot / relativeSourceArtifactPath;
    const std::string relativeNativeArtifactPath =
        packageEntry.kind == "native"
            ? (useHostedServicePaths
                   ? contentAddressedRegistryArtifactPath(nativeArtifactDigest)
                   : normalizeRelativePath((std::filesystem::path("packages") /
                                            packageEntry.packageNamespace /
                                            packageEntry.packageName /
                                            packageEntry.version /
                                            publishedTarget)
                                               .lexically_normal()
    ...
    const std::filesystem::path nativeArtifactDir =
        packageEntry.kind == "native"
            ? registryRoot / relativeNativeArtifactPath
            : std::filesystem::path{};
    ...
    if (existingRecordIt != records.end()) {
        ...
        if (existingRecordIt->license != packageEntry.license ||
            existingRecordIt->kelvraRuntime != packageEntry.kelvraRuntime) {
            outError = "Registry already contains '" + packageEntry.packageId +
                       "@" + packageEntry.version +
                       "' with different published metadata.";
            return false;
        }
    ...
    if (!useHostedServicePaths) {
        ...
        } else if (!copyDirectoryRecursive(sourceArtifactSource, sourceArtifactDir,
                                           outError)) {
            return false;
        }
        if (packageEntry.kind == "native") {
            if (fileExists(nativeArtifactDir)) {
                ...
                if (!existingDigest.empty() &&
                    existingDigest != nativeArtifactDigest) {
                    outError = "Registry artifact path '" +
                               nativeArtifactDir.string() +
                               "' already exists with different contents.";
                    return false;
                }
    ...
    if (existingRecordIt == records.end()) {
        ...
        record.license = packageEntry.license;
        record.kelvraRuntime = packageEntry.kelvraRuntime;
        if (packageEntry.kind == "native") {
    ...
    const std::filesystem::path& uploadedSourceArtifactDir =
        useHostedServicePaths ? sourceArtifactSource : sourceArtifactDir;
    if (!uploadDirectoryToUrl(uploadedSourceArtifactDir,
    ...
    if (packageEntry.kind == "native" &&
        !uploadDirectoryToUrl(useHostedServicePaths ? nativeArtifactSource
                                                    : nativeArtifactDir,
                              joinUrlPath(registryLocation.remoteRootUrl,
                                          relativeNativeArtifactPath),
                              token, outError)) {
    ...
    std::vector<std::string> indexWriteHeaders;
    if (hostedService.enabled && hostedService.conditionalIndexUpdates) {
        if (!existingIndex.remoteIndexEtag.empty()) {
            indexWriteHeaders.push_back("If-Match: " +
                                        existingIndex.remoteIndexEtag);
        } else {
            indexWriteHeaders.push_back("If-None-Match: *");
        }
    }

    HttpResponseMetadata indexWriteResponse;
    if (!uploadFileToUrlDetailed(indexPath, registryLocation.remoteIndexUrl, token,
                                 indexWriteHeaders, &indexWriteResponse,
                                 outError)) {
        if (indexWriteResponse.statusCode == 412) {
            outError = "Hosted publish failed because registry index.toml changed on the server. Rerun 'kelvra publish'.";
        } else if (indexWriteResponse.statusCode == 409) {
            outError =
                "Hosted publish failed because the registry rejected the index update as a conflict.";
        } else if (indexWriteResponse.statusCode == 401) {
            outError = "Hosted publish failed because registry credentials were rejected.";
        }
        return false;
    }
    return true;
...
bool trustProjectRegistry(const std::string& projectRoot,
    ...
    const int trustSourceCount =
        ...
        (bootstrap ? 1 : 0) + (refresh ? 1 : 0);
    if (trustSourceCount != 1) {
        outError =
            "Registry trust requires exactly one of --key, --key-file, --bootstrap, or --refresh.";
        return false;
    }
    ...
    std::vector<std::string> previousTrustedKeyIds;
    if (const UserRegistryProfile* existingProfile =
            findUserRegistryProfile(userConfig, normalizedIndex);
        existingProfile != nullptr) {
        if (!parseTrustedKeyIds(existingProfile->trustedKeys, previousTrustedKeyIds,
                                outError)) {
    ...
    RegistryTrustedKey trustedKey;
    std::vector<std::string> replacementTrustedKeys;
    if (refresh) {
        EffectiveRegistryProfile effectiveProfile;
        if (!resolveEffectiveRegistryProfile(projectRoot, *registry, false, false,
                                             effectiveProfile, outError)) {
            return false;
        }
        if (effectiveProfile.config.trustedKeys.empty()) {
            outError = "Registry '" + registryAlias +
                       "' cannot refresh trust because no trusted_keys are configured yet.";
            return false;
        }

        bool found = false;
        LoadedRegistryTrustMetadata metadata;
        if (!loadRegistryTrustMetadata(projectRoot, effectiveProfile, false, found,
                                       metadata, outError)) {
            return false;
        }
        if (!found) {
            outError = "Registry '" + registryAlias +
                       "' does not publish registry-trust.v1 for trust refresh.";
            return false;
        }

        EffectiveRegistryProfile refreshedProfile = effectiveProfile;
        if (!applyRegistryTrustMetadata(metadata, refreshedProfile, outError)) {
            return false;
        }
        replacementTrustedKeys = refreshedProfile.config.trustedKeys;
        outResult.trustedKeyIds = refreshedProfile.trustedKeyIds;
        outResult.revokedKeyIds = refreshedProfile.revokedKeyIds;
    } else if (bootstrap) {
        if (!isRemote) {
            outError = "Registry '" + registryAlias +
                       "' does not use a hosted http(s) index.";
        ...
        std::filesystem::path tempDir;
        if (!createTemporaryDirectory("kelvra-registry-trust", tempDir, outError)) {
            return false;
        }
        ScopedPathCleanup cleanup{tempDir};
    ...
    UserRegistryProfile* profile =
        upsertUserRegistryProfile(userConfig, normalizedIndex);
    if (refresh) {
        profile->trustedKeys = sortedUniqueStrings(std::move(replacementTrustedKeys));
    } else {
    ...
    if (!parseTrustedKeyIds(profile->trustedKeys, outResult.trustedKeyIds,
                            outError)) {
        outError = "Stored registry '" + normalizedIndex +
                   "' has invalid trusted_keys: " + outError;
        return false;
    }
    outResult.revokedKeyIds =
        sortedUniqueStrings(std::move(outResult.revokedKeyIds));

    const std::unordered_set<std::string> previousKeyIds(
        previousTrustedKeyIds.begin(), previousTrustedKeyIds.end());
    const std::unordered_set<std::string> currentKeyIds(
        outResult.trustedKeyIds.begin(), outResult.trustedKeyIds.end());
    for (const std::string& keyId : outResult.trustedKeyIds) {
        if (previousKeyIds.find(keyId) == previousKeyIds.end()) {
            outResult.addedKeyIds.push_back(keyId);
        }
    }
    for (const std::string& keyId : previousTrustedKeyIds) {
        if (currentKeyIds.find(keyId) == currentKeyIds.end()) {
            outResult.removedKeyIds.push_back(keyId);
        }
    }
    outResult.addedKeyIds =
        sortedUniqueStrings(std::move(outResult.addedKeyIds));
    outResult.removedKeyIds =
        sortedUniqueStrings(std::move(outResult.removedKeyIds));
    return writeUserRegistryProfiles(userConfig, outError);
}
...
bool ensureProjectPackagesInstalled(const std::string& projectRoot,
    ...
    if (manifest.dependencies.empty() &&
        ...
        (!options.includeBuildDependencies ||
         manifest.buildDependencies.empty())) {
        return true;
    }
        ...
        if (loadProjectPackageRegistry(projectRoot, installedEntries, registryError)) {
            ...
            for (const auto& entry : installedEntries) {
                if (entry.kind != "native" || entry.sourceType != "registry") {
                    // continue evaluating dependency groups below
                } else if (entry.selectedTarget != desiredTarget) {
                    targetMismatch = true;
                    break;
                }
                ...
                const std::vector<std::string> groups =
                    sortedUniqueStrings(entry.dependencyGroups);
                if (std::find(groups.begin(), groups.end(), "dev") !=
                    groups.end()) {
                    hasDevGroup = true;
                    if (!options.includeDevDependencies) {
                        groupMismatch = true;
                ...
                if (std::find(groups.begin(), groups.end(), "build") !=
                    groups.end()) {
                    hasBuildGroup = true;
                    if (!options.includeBuildDependencies) {
                        groupMismatch = true;
            ...
            if (!groupMismatch && options.includeDevDependencies &&
                !manifest.devDependencies.empty() && !hasDevGroup) {
                groupMismatch = true;
            }
            if (!groupMismatch && options.includeBuildDependencies &&
                !manifest.buildDependencies.empty() && !hasBuildGroup) {
                groupMismatch = true;
            }
        }

        const auto registryTime = std::filesystem::last_write_time(registryPath, ec);
        if (!ec && !targetMismatch && !groupMismatch) {
...
bool auditProjectPackages(const std::string& projectRoot,
    ...
    if (registryAliases.empty()) {
        outReport = "No registry-sourced packages found in kelvra.lock.";
        return true;
    }
    ...
    for (const std::string& alias : registryAliases) {
        ...
        if (registry == nullptr) {
            outError = "kelvra.lock references unknown registry '" + alias + "'.";
            return false;
        }
...
bool collectActiveCacheReferences(const std::string& projectRoot,
                                  std::unordered_set<std::string>& outRegistryArtifactDirs,
                                  std::unordered_set<std::string>& outGitResolvedDirs,
                                  std::string& outError) {
    outRegistryArtifactDirs.clear();
    outGitResolvedDirs.clear();

    std::vector<PackageRegistryEntry> installedEntries;
    if (!loadProjectPackageRegistry(projectRoot, installedEntries, outError)) {
        const std::filesystem::path registryPath =
            installRegistryPath(std::filesystem::path(projectRoot));
    ...
    const std::filesystem::path registryArtifactsRoot =
        canonicalOrLexical(registryCacheRoot() / "artifacts");
    const std::filesystem::path gitResolvedRoot =
        canonicalOrLexical(gitCacheRoot() / "resolved");

    for (const auto& entry : installedEntries) {
        if (!entry.artifactPath.empty()) {
            const std::filesystem::path artifactPath =
                canonicalOrLexical(std::filesystem::path(entry.artifactPath));
            if (pathIsWithin(artifactPath, registryArtifactsRoot)) {
                outRegistryArtifactDirs.insert(artifactPath.string());
            }
        }
        if (entry.sourceType == "git" && !entry.sourcePath.empty()) {
            const std::filesystem::path sourcePath =
                canonicalOrLexical(std::filesystem::path(entry.sourcePath));
            if (pathIsWithin(sourcePath, gitResolvedRoot)) {
                outGitResolvedDirs.insert(sourcePath.string());
            }
        }
    }
...
bool pruneLeafDirectories(const std::filesystem::path& root,
                          const std::unordered_set<std::string>& preserved,
                          bool dryRun, CachePruneStats& outStats,
                          std::string& outError) {
    std::error_code ec;
    if (!std::filesystem::exists(root, ec) || ec) {
        return true;
    ...
    for (std::filesystem::directory_iterator it(root, ec), end; !ec && it != end;
        ...
        for (std::filesystem::directory_iterator child(it->path(), ec), childEnd;
             !ec && child != childEnd; child.increment(ec)) {
            if (!child->is_directory(ec) || ec) {
                continue;
            }
            for (std::filesystem::directory_iterator grandchild(child->path(), ec),
                 grandchildEnd;
                 !ec && grandchild != grandchildEnd; grandchild.increment(ec)) {
                if (!grandchild->is_directory(ec) || ec) {
                    continue;
                }
                const std::string candidate = canonicalOrLexical(grandchild->path());
                if (preserved.find(candidate) != preserved.end()) {
                    continue;
                }
                if (!dryRun &&
                    !removePathIfExists(std::filesystem::path(candidate), outError)) {
                    return false;
                }
                ++outStats.removedCount;
                outStats.removedPaths.push_back(candidate);
            }
        }
    }
    if (ec) {
        outError = "Could not enumerate cache root '" + root.string() + "'.";
        return false;
    }
    return true;
}

bool pruneImmediateDirectories(const std::filesystem::path& root,
                               const std::unordered_set<std::string>& preserved,
                               bool dryRun, CachePruneStats& outStats,
                               std::string& outError) {
    std::error_code ec;
    ...
    for (std::filesystem::directory_iterator it(root, ec), end; !ec && it != end;
        ...
        const std::string candidate = canonicalOrLexical(it->path());
        if (preserved.find(candidate) != preserved.end()) {
            continue;
        }
        if (!dryRun &&
...
bool pruneSecondLevelDirectories(const std::filesystem::path& root,
                                 const std::unordered_set<std::string>& preserved,
                                 bool dryRun, CachePruneStats& outStats,
                                 std::string& outError) {
    std::error_code ec;
    ...
    for (std::filesystem::directory_iterator it(root, ec), end; !ec && it != end;
        ...
        for (std::filesystem::directory_iterator child(it->path(), ec), childEnd;
            ...
            const std::string candidate = canonicalOrLexical(child->path());
            if (preserved.find(candidate) != preserved.end()) {
                continue;
            }
            if (!dryRun &&
...
bool pruneProjectCaches(const std::string& projectRoot,
    ...
    std::unordered_set<std::string> preservedRegistryArtifactDirs;
    std::unordered_set<std::string> preservedGitResolvedDirs;
    if (!collectActiveCacheReferences(projectRoot, preservedRegistryArtifactDirs,
                                      preservedGitResolvedDirs, outError)) {
        return false;
    }

    CachePruneStats stats;
    if (!pruneLeafDirectories(userCacheRoot() / "local", {}, options.dryRun, stats,
                              outError) ||
        !pruneLeafDirectories(projectFallbackCacheRoot(std::filesystem::path(projectRoot)),
                             {}, options.dryRun, stats, outError) ||
        !pruneSecondLevelDirectories(registryCacheRoot() / "artifacts",
                                     preservedRegistryArtifactDirs,
                                     options.dryRun, stats, outError) ||
        !pruneImmediateDirectories(gitCacheRoot() / "working", {}, options.dryRun,
                                   stats, outError) ||
        !pruneImmediateDirectories(gitCacheRoot() / "resolved",
                                   preservedGitResolvedDirs, options.dryRun,
                                   stats, outError)) {
        return false;
    }

    std::ostringstream report;
    report << (options.dryRun ? "Would remove " : "Removed ")
           << stats.removedCount << " cache director";
    report << (stats.removedCount == 1 ? "y" : "ies") << ".";
    for (const std::string& path : stats.removedPaths) {
        report << "\n" << path;
    }
    outReport = report.str();
    return true;
