...
Check the installed runtime and native package ABI before diagnosing package
...
`mog version` and `mog -V` print the same version line.
...
`mog test` and `mog build` run the corresponding root entry from the
`[scripts]` table in `mog.toml`, installing development and build dependencies
first. `mog run` installs normal dependencies when needed before executing the
